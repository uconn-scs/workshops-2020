...
The aims of this workshop series are to i) introduce participants to statistical and data visualization tools available in R, ii) promote the use or R by participants in their own work and research, and iii) promote reproducible research through making all code and workshop content freely available to all participants. 
...
For the first step, you can use the `install.packages` function. For these workshops, we will install serveral packages. To begin, install `github`:
...
You can check which packages you have loaded by typing `installed.packages()`
...
To make the packages available to in the current session, you need to use the `library()` function. Note this will only work on installed packages. 
...
For the purposes of these workshops you will need to install all of the packages in the following list: \newline
...
* More about Plots
    + Scatterplots
    + Plotting large numbers of points
    + Adding linear and non-linear trendlines
* Scatterplot matrix: comparing multiple groups
* How to decorate your plots
    + Title
    + Label
    + Font size
    + Dot size
* Putting plots together: 'cowplot' and 'gridExtra' for combining plots
* Multivariate continuous data (longditudinal data)
    + Plotting time-series data
    + Plotting spatial data
    + 3D and interactive plots with 'ggplotly'
